...
块级元素默认的话是content占满一行，所以一旦content的width被重新设定了的话，浏览器会默认的让margin来补全这一行，也就是设置margin-right让这个盒子把这一行依旧占满，如果此时margin又设为自动的话那么浏览器会默认地同时设置margin-left和margin-right让盒子把这一行占满的同时content内容也移到中间去了，达到了好看的效果



块级元素和行内元素最根本的区别在于行内元素盒子模型的content的宽度和高度是不能调的，而是根据其中内容的多少自动设置的

块级元素就算是变成了内联元素 但是它们的外边框还是不能连在一起呢~



垂直外边距的压缩发生在两个盒子中一个盒子的底部和一个盒子的顶部的margin值不同的时候，这样在显示的时候会默认使用大的一方，因为这样看起来开阔一点 会更美观9

新的css属性border-box

tr标签的的盒子模型和一般的标签的盒子模型不太一样哦